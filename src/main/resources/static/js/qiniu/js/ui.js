...
function FileProgress(file, targetID) {
  ...
  if (!this.fileProgressWrapper.length) {
    ...
    var progressDel = $("<td/>");
      progressDel.addClass('progressDel').css('cursor','pointer').text('×');
    ...
    progressCancel.show().addClass('progressCancel').css('cursor','pointer').text('×');
...
FileProgress.prototype.setComplete = function(up, info) {
  ...
  if (res.url) {
    ...
    str = "<div><strong>链接地址:</strong><a href=" + url + " target='_blank' > " +
      link + "</a></div>";
  }
  ...
  if (!isImg) {
    showImg.attr('src', '/js/qiniu/images/default.png');
