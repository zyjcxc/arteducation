function uploadWithOthers(token, putExtra, config, domain) {
    ...
    qiniu.getUploadUrl(config, token).then(function(res){
        ...
        var uploader = new plupload.Uploader({
            ...
            init: {
                ...
                FilesAdded: function(up, files) {
                    ...
                    var b = false;
                    for (var f1 in putExtra.mimeType) {
                        if (putExtra.mimeType[f1].indexOf(suffix) > -1) {
                            b = true;
                            break;
                        }
                    }
                    ...
                    if ($("#fsUploadProgress2").find("tr").length > 0) {
                        $("#fsUploadProgress2").find("tr").remove();
                    }
                    var id = files[0].id;
                    // 添加上传dom面板
        ...
        uploader.bind("BeforeUpload", function(uploader, file) {
            ...
            putExtra.params["suffix"] = key.split(".")[1];
            ...
            var directUpload = function() {
                ...
                multipart_params_obj.key =  new Date().getTime() + "." + putExtra.params["suffix"];
                uploader.setOption({
                    url: uploadUrl,
                    multipart: true,
        ...
        function uploadFinish(res, name, board) {
            ...
            $(board)
                ...
                .addClass("hide");
            $(board)
                .find(".control-container").addClass("hide");
            /*$(board)
            ...
            if (name && name.match(/\.(JPG|jpg|jpeg|png|gif|mp4|wmv|MP4|WMV)$/)) {
