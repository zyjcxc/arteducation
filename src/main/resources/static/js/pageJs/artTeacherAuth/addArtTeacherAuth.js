...
pubAccount_add = (function ($, w) {
    ...
    function addEventListeners() {
        ...
        $$.onJq(_self.getPageBtn().back, "click", function () {
            $$.goTo({
                url: WEB_CONFIG._page.ART_TEACHER_AUTH_LIST_PAGE
            });
        });
    ...
    function save() {
        ...
        $.ajax({
            type : 'post',
            url : WEB_CONFIG._action.ART_TEACHER_AUTH_ACTION,
            contentType: "application/json; charset=utf-8",
            data : JSON.stringify($PAGE_FORM.serializeObject()),
            async: false,
            success : function(data) {
                $$.goTo({
                    url: WEB_CONFIG._page.ART_TEACHER_AUTH_ADD_PAGE
                });
            },
            error: function (res) {
    ...
    function initPageData() {
        ...
        $.ajax({
            type : 'get',
            url : WEB_CONFIG._action.ART_TEACHER_AUTH_ACTION + '/' + _self.get$Scope().id,
            async : false,
            success : function(data) {
                _self.getPageDom().id.val(data.id);
                _self.getPageDom().name.val(data.name);
                _self.getPageDom().sex.val(data.sex);
                _self.getPageDom().vatime.val(data.vatime);
