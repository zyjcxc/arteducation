...
classification_list = (function ($, w) {
				...
				{"data" : "updatetime", "defaultContent" : ""},
                    {
                        ...
                        "render": function (data, type, row) {
                            var id = row['id'];
                            // 编辑地址
                            var edit = buttonEdit(WEB_CONFIG._page.ART_DOWN_LOAD_PAGE_WITH_PARAMS({
                                id : id,
    ...
    function addEventListeners() {
        ...
        $$.onJq(_self.getPageBtn().add, "click", function () {
            $$.goTo.call(_self, {
                url : WEB_CONFIG._page.ART_DOWN_LOAD_PAGE_WITH_PARAMS({
                        returnUrl :  _self.get$Scope().cur_page
                })
            });
        });
...
function updateState(id, state, operator) {
    ...
    layer.confirm('确定要' + operator + '吗？', {
        ...
        $.ajax({
            type : 'DELETE',
            url : WEB_CONFIG._action.ART_FILE_LIST + '/' + id ,
