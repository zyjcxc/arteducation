...
classification_add = (function ($, w) {
    ...
    function hasContent() {
        var arr = [];
        arr.push(UM.getEditor('addDownloadEditor').hasContents());
        UM.getEditor('addDownloadEditor').focus();
        return (arr.join("\n"));
    }
    function setContent(isAppendTo) {
        UM.getEditor('addDownloadEditor').setContent(isAppendTo);
    }
    function save() {
        var _self = this;
        ...
        $.ajax({
            type : 'post',
            url : WEB_CONFIG._action.ART_FILE_LIST,
    ...
    function update() {
        ...
        console.log(params)
        if(!params.url || params.url === ''){
            layer.msg("请上传文件！", {shift: -1, time: 3000});
            return;
        }
